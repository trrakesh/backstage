...
import { BackstageJWTPayload } from './type';
...
export class CustomJWTTokenValidator implements TokenValidator {
    ...
    async invalidateToken(jwt: string): Promise<void> {
        await this.isValid(jwt);
        const { sub } = parseJwtPayload(jwt);
    ...
    async isValid(jwt: string): Promise<boolean> {
